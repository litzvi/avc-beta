...
//				+ "left join sf.warehouseLocation warehouseLocation "
	...
	@Query("select new com.avc.mis.beta.dto.report.RawQcRow( "
			...
			+ "( "
				+ "coalesce(def.scorched, 0) + coalesce(def.deepCut, 0) + coalesce(def.offColour, 0) "
				+ " + coalesce(def.shrivel, 0) + coalesce(def.desert, 0) + coalesce(def.deepSpot, 0)"
			+ ")/ti.sampleWeight, "
			+ "("
				+ "coalesce(dam.mold, 0) + coalesce(dam.dirty, 0) + coalesce(dam.lightDirty, 0) "
				+ " + coalesce(dam.decay, 0) + coalesce(dam.insectDamage, 0) + coalesce(dam.testa, 0) "
			+ ")/ti.sampleWeight) "
