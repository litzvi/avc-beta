...
public interface ReceiptRepository extends BaseRepository<Receipt> {
	...
	@Query("select new com.avc.mis.beta.dto.query.ReceiptItemWithStorage( "
			...
			+ "sf.sampleContainerWeights, sf.sampleContainerWeight, sf.sampleWeights, sf.numberOfSamples, sf.avgTestedWeight, "
//			+ "CASE "
...
//		+ ", sf.ordinal " //already sorted in dto for storages
	...
	@Query("select new com.avc.mis.beta.dto.view.ReceiptItemRow( "
				...
				+ "r.recordedTime, "
				+ "SUM(sf.unitAmount * sf.numberUnits * uom.multiplicand / uom.divisor), i.defaultMeasureUnit, "
			...
			+ "from Receipt r "
				...
				+ "join r.processItems pi "
					...
					+ "join pi.storageForms sf "
						+ "join sf.group sf_group "
							+ "join UOM uom "
								+ "on uom.fromUnit = sf_group.measureUnit and uom.toUnit = i.defaultMeasureUnit "
